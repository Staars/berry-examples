...
def parse_map_file(map_file):
    ...
    with open(map_file, 'r', encoding='UTF-8') as file:
        while (line := file.readline()):
            ...
            if "ulp_riscv_run" in line:
                ulp_type = "RISCV"
    return global_vars,ulp_type
...
def print_output(code,code_b64,code_size,global_vars,ulp_type):
    print("### Global vars (including function labels):")
    for var in global_vars:
        rtc_addr = int((int(var.split(" ")[-1],0)-0x50000000)/4)
        if rtc_addr == 0 and ulp_type == "FSM":
    ...
    print("")
    print("# ULP-Coprocessor-type:", ulp_type)
...
def main(args):
    ...
    global_vars,ulp_type = parse_map_file(map_file)
    print_output(code, code_b64, code_size, global_vars,ulp_type)
